# SI contains the sampled indices and SP contains their corresponding
# probabilities such that SP [i] = P [SI[i]] for all i ≤ Ns .
def sample(P, gamma, b):
    ...
    while(i < len(P)):
        ...
        i = i+1
    S.append((P[-1], len(P)))
    # the sampling process returns a sampled probability distribution (SI,SP) of size Ns
    return S
...
def sampleMerge(S1, S2, gamma, b):
    ...
    for i in range(1, len(P)):
        ...
        if jp+1 < n2 and ip+1 < n1:
            # print(U)
            # print((U[-1],S2[jp+1][1],S2[jp][1],S1[ip+1][1],S1[ip][1]))
